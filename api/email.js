...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Pro early access capture ────────────────────────────────────────
    if (action === 'pro_capture') {
...
<div style="max-width:500px;margin:0 auto;background:#fff;">
  <div style="background:#1a2744;padding:24px 32px;">
    <div style="color:#fff;font-size:18px;font-weight:700;">CivicScope Pro — Early Access Signup</div>
  ...
  <div style="padding:28px 32px;">
    <table style="width:100%;border-collapse:collapse;font-size:14px;">
      ...
      ${name ? `<tr style="background:#f9f6f3;"><td style="padding:8px 6px;color:#888;">Name</td><td style="padding:8px 6px;color:#222;">${name}</td></tr>` : ''}
    </table>
  </div>
</div>
</body></html>`;
      ...
      const result = await sendEmail(RESEND_API_KEY, {
        from: FROM_EMAIL,
        to: [KEITH_EMAIL],
        subject: `Pro Early Access: ${email}`,
        html: notifyHtml
      });
      return res.status(200).json({ sent: true, id: result.id });
    }
...
<div style="max-width:600px;margin:0 auto;background:#fff;">
  ...
  <div style="padding:40px;">
    ...
    <div style="margin: 28px 0 0; padding: 24px 28px; background: #f2ede4; border-radius: 6px;">
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
        ${data.timeline.phases.map(p => `
        <tr>
          <td style="padding: 8px 0; border-bottom: 1px solid #e7e2d8; color: #44403c;">${p.phase}</td>
          <td style="padding: 8px 0; border-bottom: 1px solid #e7e2d8; color: #c2410c; font-weight: 600; text-align: right;">${p.duration}</td>
        </tr>`).join('')}
      </table>
      <p style="font-size: 12px; color: #78716c; margin: 14px 0 0; line-height: 1.5;">
        ${data.timeline.total} total &mdash; ${data.timeline.note}
      </p>
    </div>
    ` : ''}
  ...
  <div style="background:#f5f0eb;padding:20px 40px;border-top:1px solid #e8e0d5;">
    ...
    <p style="color:#bbb;font-size:10px;margin:4px 0 0;">CivicScope · PO Box 69, Nappanee, IN 46550</p>
  </div>
</div>
</body>
</html>`;
