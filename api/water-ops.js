...
/* The bytes themselves, server-side. Used when a filing is recorded FROM a generation: the OIC
   sent the file the product built, so the product already holds the bytes and asking her to
   upload them back would be asking her to prove something the product did. */
...
/* A generation row as the review page wants it — everything but the `filed` blob. */
const generationCard = (g) => ({
  id: g.id, year: g.report_year, month: g.report_month,
  generated_at: g.generated_at, generated_by: g.generated_by, origin: g.origin,
  workbook_name: g.workbook_name, workbook_sha256: g.workbook_sha256, workbook_bytes: g.workbook_bytes,
  stats: g.stats || {}, filing_id: g.filing_id,
});
...
export default async function handler(req, res) {
  ...
  /* `generation_workbook` (2026-09-11) is the same public record one step earlier: the workbook
     the product built from numbers `month` already serves. */
  const READ_ONLY = new Set(['profile', 'month', 'preview', 'filings', 'filing_workbook', 'generation_workbook']);
  ...
  try {
    ...
    switch (action) {
      ...
      case 'month': {
        ...
        /* Every workbook the product has handed over for this month, newest first (2026-09-11).
           Read soft: a month with no generations is the ordinary state, and the table not existing
           yet on a deployment that beat its migration must not take the OIC's page down. */
        let generations = [];
        try {
          const g = await sb(
      ...
      /* ---- record that a workbook was handed over (2026-09-11) ------------------------------------
         Called by api/build-mor.py after every fill, server-to-server with the ops code — never by
         the page, so a script or a curl that generates a month leaves the same trace the button
         does. The browser's session cookie is forwarded by the Python route, which is why `actor`
         can be resolved here: the row names the signed-in person when there is one and says
         nothing when there is not, rather than trusting a name the caller asserts.
         Admin-gated (the code, or an admin sign-in): this is an assertion about the record,
         written by the product about itself. Nobody at the village holds the code, and nobody at
         the village should be able to write "generated by Michelle" into the log by hand. */
      case 'record_generation': {
        ...
        /* Under its own prefix so a generated draft can never be mistaken for a filed workbook by
           ...
           one 409 rather than a second copy. */
        const path = `${wssn}/generated/${y}-${String(m).padStart(2, '0')}-${sha.slice(0, 8)}.xls`;
        const up = await putWorkbook(path, bytes);
        if (!up.ok) return bad(res, 502, `could not store the workbook (${up.status}): ${up.msg}`);
        const who = actor ? (actor.name || actor.email) : null;
        const ins = await sb('water_mor_generations', {
          method: 'POST', headers: { Prefer: 'return=representation' },
          body: JSON.stringify([{
            supply_id: p.supply.id, report_year: y, report_month: m,
            generated_by: who, user_id: actor ? actor.id : null,
            origin: body.origin === 'script' ? 'script' : 'review',
            workbook_name: String(body.workbook_name || `${y}-${String(m).padStart(2, '0')}.xls`),
            workbook_bucket: MOR_BUCKET, workbook_path: path, workbook_sha256: sha, workbook_bytes: bytes.length,
            stats: body.stats && typeof body.stats === 'object' ? body.stats : {},
            filed: body.filed && typeof body.filed === 'object' ? body.filed : {},
          }]),
        });
        return res.status(200).json({ ok: true, id: ins[0].id, sha256: sha, generated_by: who, generated_at: ins[0].generated_at });
      }
      ...
      case 'generation_workbook': {
        ...
        const rows = await sb(
          `water_mor_generations?id=eq.${id}&supply_id=eq.${p.supply.id}` +
            `&select=id,workbook_name,workbook_path,workbook_sha256&limit=1`
        );
        const g = rows && rows[0];
        if (!g) return bad(res, 404, 'no such generation for this supply');
        const url = await signWorkbook(g.workbook_path, 900);
        if (!url) return bad(res, 502, 'could not sign the workbook');
        return res.status(200).json({ ok: true, name: g.workbook_name, sha256: g.workbook_sha256, url, expires_in: 900 });
      }
      ...
      /* ---- record that a month was filed --------------------------------------------------------
         ...
         AND, SINCE 2026-09-11, THE DOOR THE OIC USES FROM THE PAGE. Two shapes:
           * `workbook_b64` — the file she actually sent, uploaded back. The page has already run
             it through build-mor's `extract`, so `cover` and `filed` arrive read from its cells.
           * `generation_id` and no bytes — she sent the generated file as it was. The product
             holds those bytes and what they say, so nothing is re-uploaded; the date and the
             comments come from the form, because the generated Cover has neither.
         Either way `generation_id` links the generation to the filing it became, so the panel can
      ...
      case 'record_filing': {
        ...
        /* The generation this filing came from, if any. Must be this supply's and this month's —
           a filing for August recorded against July's generation would be a lie in both rows. */
        let gen = null;
        if (body.generation_id) {
          const gid = String(body.generation_id);
          if (!/^[0-9a-f-]{36}$/i.test(gid)) return bad(res, 400, 'generation_id is not an id');
          const rows = await sb(
            `water_mor_generations?id=eq.${gid}&supply_id=eq.${p.supply.id}&select=*&limit=1`
          );
          gen = rows && rows[0];
          if (!gen) return bad(res, 404, 'no such generation for this supply');
          if (gen.report_year !== y || gen.report_month !== m) {
            return bad(res, 400, `that generation is ${gen.report_year}-${String(gen.report_month).padStart(2, '0')}, not ${y}-${String(m).padStart(2, '0')}`);
          }
        }
        ...
        const ins = await sb('rpc/water_replace_row', {
          ...
          body: JSON.stringify({
            ...
            p_row: {
              ...
              workbook_name: String(body.workbook_name || (gen && gen.workbook_name) || `${y}-${String(m).padStart(2, '0')}.xls`),
              ...
              /* A filing recorded from a generation is a PRODUCT filing whatever the caller says —
                 that is the first value of `source` the product has ever written, and it is what
                 keeps the first report filed from here distinguishable from the seven before it. */
              source: gen ? 'product'
                : (['backfill', 'product', 'import'].includes(body.source) ? body.source : 'backfill'),
