...
- Gate + harness scripts live in `scripts/` — **version-controlled but deliberately NOT in the
  ...
  Local suite: `node scripts/test-deploy-harness.js` (114 checks, fully mocked — no commit,
  no deploy, no network egress).
...
- **`scripts/test-deploy-harness.js` aborts partway with exit 255 (found 2026-08-08).** Two consecutive runs died at a *different* scenario each time (`F3-nogate-not-success`, then `R3-F1-resume-never-commits-worktree-bytes`) with **no FAIL assertions** — output simply stops after a section header. A sibling node run in the same session printed the Windows libuv abort `Assertion failed: !(handle->flags & UV_HANDLE_CLOSING), src\win\async.c:94`, so the likely cause is the harness's `proc.kill()` of its mock servers, not any assertion. **The harness therefore cannot currently certify the deploy script it exists to certify** — a partial run that ends in an abort looks like a pass to anyone reading the last screenful. Reproduce, then either fix the teardown or make the runner exit non-zero with an explicit "N scenarios never ran" line. Unrelated to the pool work that surfaced it.
