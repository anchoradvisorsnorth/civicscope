...
- /golf + /golf/picks — **RESURRECTED 2026-07-15** for The Open (Royal Birkdale, Jul 16–19). Hidden personal pages (not a CivicScope product): friends' prize-money pool, now **DB-backed** instead of hardcoded picks — `golf_pools` table (CS Supabase, RLS on, created via Management API), [api/golf-pool.js](api/golf-pool.js) (GET public / POST gated by `GOLF_POOL_CODE` Vercel env var = the commissioner code), `golf/index.html` (tracker: official $17.75M payout table, Monte Carlo odds, ESPN event pinned `401811957`) + `golf/picks.html` (commissioner picks UI w/ save-draft + 🔒 lock flow; Mike = commissioner). TEMPORARY VM cron `golf-lock-watch` (every 10 min) emails Keith when picks lock — **remove after the tournament** (crontab entry + `~/golf-lock-watch.js` + `~/.golf-lock-notified`). This pool group recurs (US Open June, The Open July, **football in the fall**) — see memory `project_friends_pools`. First retirement: 2026-06-22 (US Open version, static HTML, won by MARK).
- /football + /football/picks + /football/commish — **v1 BUILT 2026-07-15** (before the season, so Mike can request tweaks). Weekly ATS pool: commissioner builds a slate (ESPN NFL + CFB scoreboards w/ live DraftKings lines, manual-spread fallback), **spreads freeze on slate lock** (pick deadline = first kickoff; lock emails each player their PIN via Resend `pool@civicscope.io`), players pick winners ATS at `/football/picks` (name + 4-digit PIN; picks hidden from others until deadline — enforced server-side), pool home shows live ATS scoring (1 pt win / ½ push), weekly winner (commissioner finalizes), season standings. Storage: `football_pools` (CS Supabase, slug-keyed jsonb: `config` + `2026-wNN`, RLS on); [api/football-pool.js](api/football-pool.js); commissioner code = `FOOTBALL_POOL_CODE` Vercel env var. Demo Week 1 seeded with 4 real NFL week-1 games (real frozen lines) for Mike's review.
...
## Recent Changes (July 15, 2026) — credits incident CLOSED (auto-reload on) + pools built
- **Anthropic credits hit $0 (2nd time; 1st was June 16)** at ~8:50 PM — cs-health caught it within the hour, all 3 tools down ~2h at night, zero known user impact. Keith **enabled auto-reload in the console: reload to $15 when balance reaches $5** (+ existing $1,000/mo spend limit) — the recurring depletion failure mode is permanently retired. Real measured burn: ~$9–25/mo across all Anthropic consumers (tools, cs-health, watchers, CRM enrichment, AAN monitor). Smoke-verified recovered same night.
- **Golf pool resurrected** (The Open, 15 picks/player after Keith's correction) + **Football pool v1 built** — see the /golf and /football entries in Routing above. Twilio account created for pool SMS onboarding (compliance profile pending review; tracker row added).
- Commish game-browser UI: NFL/College tabs + 60vh scroll + team filter (Keith feedback, same-night deploy).

## Recent Changes (July 16–17, 2026) — football pool refinements + Twilio pivot
- **Football pool tweaks (all deployed + verified):** (1) **reveal-on-all-locked** — picks now unmask the moment the LAST player locks (or first kickoff, whichever's first), not just at deadline; server returns `revealed` flag; home board shows a "waiting on: X" list. (2) **"All picks are in" email** to the group when the board completes (`notifyAllLocked`, guard flag prevents double-send; SMS later). (3) **Monday games excluded** from the commissioner's slate builder — Tue–Sun competition; filtered by **Eastern day-of-week** so SNF (Mon in UTC) stays and MNF (Tue in UTC) drops — verified against real Week 2 games. (4) Reusable **`scripts/notify-pool-change.js`** — one command emails Mike (cc Keith) a plain-English "what changed" changelog on every pool change; first one sent.
- **⚠ Deploy gotcha reconfirmed:** the CivicScope webhook served a **stale serverless-function build** — new `api/*.js` code was in GitHub but the live function didn't update until a **`vercel deploy --prod --force`**. Going forward: after any change touching an `api/*.js` function, force-deploy or verify the live endpoint — don't trust the push alone.
- **Twilio** created for pool SMS onboarding — business profile (JBK Investments LLC) REJECTED (couldn't verify Keith↔entity). Pivoting to a **Sole Proprietor** Trust Hub profile (no EIN, personal/recreational use) via a support ticket. Account SID known; awaiting profile fix → toll-free/local number → Auth Token → wire into Vercel. Tracker row added (row 30, `watch`).

## Recent Changes (July 13, 2026) — RLS lockdown COMPLETE (Supabase security alert closed)
- **Trigger:** Supabase "Action required" email (Jul 7), surfaced by the AAN Weekly monitor. Advisor scan + live test confirmed: `leads` (prospect PII), `tool_runs`, `sessions`, `tenants`, `qa_runs` had no RLS and were **readable by anyone** via the `sb_publishable_*` key shipped in the admin page of this public repo.
- **Code (commit `661ba53`, smoke+E2E passed):** `/api/admin` gained a secret-gated `read` action (whitelist: tenants/leads/sessions/tool_runs/qa_runs, path sanity checks); admin page `sbRead()` + the REST health-check now use it. **No Supabase key ships in any page.** The April 23 "reads stay client-side on the anon key" trade-off is retired.
- **SQL (Keith ran 2026-07-13, both projects):** RLS enabled on the 5 CS tables + 4 CRM tables (`cs_email_templates`, `cs_campaigns`, `cs_campaign_sends`, `contact_captures`); CRM `business_status` allow-all policy dropped; both `update_updated_at` search_paths pinned. File: `CRM/schema_rls_lockdown_2026-07-13.sql`.
- **Verified after:** anon key returns 0 rows on all 5 CS tables; live `create_session` write OK (server/service-key paths RLS-immune); business-status push OK; **advisor re-scan: 0 ERRORs / 0 WARNs on both projects** (was 6 ERRORs + 3 WARNs).

## Recent Changes (July 8, 2026 PM) — Fable fixes R3+R4 shipped
- **R3 CAN-SPAM postal address — DONE.** `CivicScope · PO Box 69, Nappanee, IN 46550` added to the campaign plain-text footer ([CRM/api/cs-campaign.js](CRM/api/cs-campaign.js) `unsubFooter`), the report-email footer ([api/email.js](api/email.js)), and the `/unsubscribe` confirm page. The last legal blocker before school-BOT outreach volume is closed.
- **R4 mobile overflow — DONE, all 6 pages verified exactly 375px.** Root fixes, not masking: landings ×3 (header now wraps + 170px wordmark at ≤600; `.problem-grid` → `minmax(0,1fr)` — the use-case card was blowing the grid track; card unstuck + media clamped), tools ×2 (150px wordmark, header-badge dropped at ≤600 — the free-banner repeats it; smaller vertical tag), hub (160px mark, non-CTA nav links hidden ≤480). **Hub Infrastructure card got its missing "Live" badge.** The wedge page (for-schools, was 540px) is now clean on a phone — unblocks superintendent outreach.
