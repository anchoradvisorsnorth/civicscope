...
**AI:** All tools → api/claude.js → **claude-sonnet-4-6**, temp 0.3. **max_tokens: 2400** on Municipal/Schools/Infrastructure (raised from 1200 on 6/16 — Schools/Infra responses run ~1,150–1,300 tokens and truncated mid-JSON at 1200); GC/other tools still 1200. `api/claude.js` pins `maxDuration: 300` (Vercel Pro max) and its retry loop is **deadline-aware** (raised from 120 + unbounded retries on 2026-09-02, `8af3cc3`). 120s was measured against TEXT estimates only: a RYC plan-set batch runs ~62s, so ONE attempt fit and a retry never could, and the function was killed mid-flight — returning a **bodyless 504** that destroyed a 30-minute Desk takeoff. Attempts are now bounded by the remaining budget and a timeout answers in JSON. (Model was `claude-sonnet-4-20250514` until it RETIRED ~June 15, 2026 → 404'd every tool; see Recent Changes June 16. **Never pin a dated/soon-retiring model — use the rolling alias.**)
...
**Generating is not filing — but generating is now RECORDED (2026-09-11, `1.6.0-waterops`,
review `v1.4.0-oic`, migration `073`).** It reads a month through the ungated `month` action and
the template out of storage. What Michelle sends EGLE is the file *after* she adds her Cover
comments — she writes them every month — so the generated bytes are never presented as the filing,
and the filed copy is recorded from what she actually sent.

⛔ **UNTIL 2026-09-11 IT HANDED OVER THE FILE AND REMEMBERED NOTHING.** Keith, with the August
reminder sent on the 7th, the 10th gone by and no way to tell whether the OIC had ever pressed the
button: *"If generated wouldn't the report show at the bottom of the page. Otherwise, we need to
tighten this up."* It did not show, because nothing recorded it — no row, no bytes, no time, no
name — and the Vercel runtime log keeps about a day, so **"has Michelle generated August?" was a
question the product could not answer about its own most important action.** (The answer, once
it could be measured: the reminder was opened, no bacti was ever added, no filing recorded — and
from today the panel itself says whether a month was generated.)

**What closes it, all three shipped together:**
- **Every generate records itself, server-side.** After the fill, `api/build-mor.py` posts the
  bytes, the stats and what the workbook *says* (read back through the extractor below) to
  `record_generation` on `api/water-ops.js`, authenticated with the ops code, **forwarding the
  browser's session cookie** so the row names the signed-in person when there is one and says
  "someone who was not signed in" when there is not. Recorded by the generator and never by the
  page, so a script or a curl leaves the same trace the button does. If recording fails the file
  is still handed over — refusing the OIC her report because an audit write failed is the wrong
  failure — but `X-Mor-Stats` carries `recorded:false` and why, and the page shows it.
  Table `water_mor_generations`; bytes in the same private bucket under `<wssn>/generated/`,
  content-hashed. `record_generation` is admin-gated (code or admin sign-in): nobody at the village
  can write "generated by Michelle" into the log by hand.
- **The panel shows it.** `month` returns `generations` (newest first); `/water/review` renders
  the newest as *"September 2026 was generated — not yet recorded as filed"* with when, by whom,
  the stats, **Open this workbook** (`generation_workbook`, a 15-minute signed link) and **Mark as
  filed**. A filed month collapses to one line: *"Filed from the workbook generated here on…"*.
- **Mark as filed — the loop's last step, BUILT.** An office write (signed-in, enrolled for the
  supply — the page sends her to the hub when she is not). The form asks for the two things the
  generated workbook cannot know — the date it went and her Cover comments — plus, optionally,
  **the file she actually sent**. With the file: the page runs it through `extract` and records
  what its cells say, the form only filling what the Cover left blank. Without it: `record_filing`
  takes `generation_id`, reads the generated bytes back out of storage, uses the `filed` the
  generation stored, and stamps `source:'product'` — the first value of `source` the product has
  ever written. Either way the generation is linked to the filing it became (`filing_id`).

**The extractor lives in `api/build-mor.py` too (`action: extract`, moved from
`scripts/extract-mor.py`, which is now a thin client like `scripts/build-mor.py`).** One cell map,
one file, both directions. Proven identical to the old script on all seven 2026 filed workbooks
before the script was cut over. ⚠ **The deploy gate caught a real defect on the first run:** the
extractor read the product's *own* July workbook back as **0 distribution samples of 23**, because
`build()` writes sample dates as text (`7/29/2026`) and the extractor only read numeric cells. A
filing recorded from a generation would have compared "23 held vs 0 filed" — a phantom divergence
on a compliance record, manufactured by the product disagreeing with itself. Fixed in `_as_date`
(text `M/D/YYYY`, `M/D`, ISO), and `verify-mor-filings.mjs` now asserts the generated July reads
back as 31 pumpage days, the distribution count written, and **no submission date** (the date is
hers to add, never ours). The gate also asserts the generation was recorded, is listed newest-first
with the served hash, opens through a signed link, and that an unauthenticated `record_generation`
is refused — 34 checks, up from 28.
...
4. ✅ **"Mark as filed" is BUILT, and every generation is recorded (2026-09-11).** Was: half built —
   ...
   attributed through the forwarded session cookie), `/water/review` shows the month as
   *generated — not yet recorded as filed* with the bytes openable, and **Mark as filed** records
   the filing from the file she sent (read by the moved-in `extract` action) or from the generated
   bytes the product already holds, stamping `source:'product'`. Detail: *Michelle generates the
   MOR herself* above. LIVE-PROOF: the first real filing recorded from the page.
...
6. 🚩 **ASK CAN PRESENT PLANNING COMMISSION MINUTES AS ADOPTED AUTHORITY** (Codex finding 10).
...
7. **`cited_collections` logs what was RETRIEVED, not what was CITED** (Codex finding 11, Low).
...
### ✅ Closed 2026-09-02 (round 3) — the write path is one transaction, migration 064

Codex's two remaining High findings said the same thing twice: `submit_reading` wrote a well-day in
**four separate PostgREST calls with no transaction**, and every boundary between them was a place
the record could be left half-made. The JS compensating rollback shipped in `3af8b2a` narrowed the
window; it could not close it, because a lambda that dies mid-sequence leaves nobody to compensate.

**`migrations/064_water_submit_txn.sql` — applied and verified in one transaction, 246 ms.**
`water_submit_reading(p_supersedes, p_reading, p_feeds, p_successor)` does supersede → insert
reading → insert feed rows → update the recomputed successor, and **asserts an affected-row count
on every statement**. `Prefer: return=minimal` answered 204 for a successful update *and* for one
that matched no row, which is how "I updated the successor" and "I updated nothing" became
indistinguishable to a caller that then reported success.

⛔ **THE FUNCTION DOES NO ARITHMETIC AND MUST NEVER LEARN ANY.** `derive.js` is the one copy of the
dose formula precisely because Centreville's daily log and its MOR disagreed for years. Porting it
into PL/pgSQL to win atomicity would recreate that defect in the last place anyone would look.
Instead the caller derives **everything first — including the successor**, whose baseline is the row
about to be inserted and is therefore knowable before it exists (`planSuccessor()` builds `prev`
from the caller's own derived values rather than reading it back). The function is a pure
transactional writer: it moves values, it never computes one. The verify pins this by asserting
`8.34`, `avail_fraction`, `ortho_factor` and `1e6` are **absent** from the installed body.

⛔ **AN UNKNOWN KEY NOW RAISES INSTEAD OF BEING DROPPED.** `jsonb_populate_record` ignores keys it
does not recognise — the same failure shape that let `dose_mgl` sit in a compliance write for a
month. Every key is checked against `information_schema.columns` and an unknown one aborts the
transaction. (This is why `kind` — derive()'s label for the caller, not a column — is stripped from
`p_feeds` before the call, and why the harness asserts that it is.)

**A successor that cannot be recomputed now refuses the whole submit, before anything is written**
...
- ✅ **`scripts/verify-water-write-path.mjs` — the gate that can see a write.** 22 checks,
  ...
  "could not run" is never legitimate. It asserts one call per visit (not four), that the call is
  the RPC, that `kind` never travels, that the successor's plant-tap sample / pressure / temp
  survive, that a changed flag set leaves a `rederived` audit entry carrying what it replaced, and
  that a rollback answers `saved:false` so the offline queue may re-send.
  ⚠ **Mutation-tested before it was trusted** — reverting the `kind` strip fails it 1/22, and
  letting an impossible successor through fails it 3/22. A harness that has never failed has not
  been shown to work.
- ⚠ **Still open, and honestly so:** the `rederived` audit entry lives in the same mutable `flags`
  jsonb as the live flags. A genuinely immutable submission-time record is a separate table and is
  **not** built. `submit_dist` and `submit_bacti` still use the old supersede-then-insert sequence
  — they write a single row with no children, so the partial-graph failure does not apply, but they
...
> **STANDING RULE — Codex findings 3 and 4 were put to Keith on 2026-09-02 and he answered NO to
> both. They are DECIDED, not deferred. Do not re-open them, do not re-surface them in a backlog,
> and do not implement either as a side effect of adjacent work.** Both were High in the review, so
> a later session will find them again from the report and should read this before acting.
>
> - **NO device binding on `/water` writes.** `submit_reading` / `submit_dist` / `submit_bacti`
>   stay open. Keith's original shape holds — *"this is the wellhouse app, it needs to be super
>   simple"*, *"each inspector should just need to add their initials"*: no login, no access code,
>   and now no signed device token either. The accepted consequence, stated plainly so nobody
>   rediscovers it as a surprise: `operator_id` is optional, its PIN is checked only when the caller
>   also supplies one, a bare `operator_initials` string is stored as given, and the filed-month and
>   correction guards do not cover today's open date — so an unauthenticated POST can create a
>   plausible current-month row in someone's initials. **What makes the record trustworthy is
>   Michelle's review and signature under 1976 PA 399, not the tablet.** That was always the design;
>   it is now the design on the record.
> - **NO acknowledgement gate on warnings.** A flag is displayed and stored, and nothing more. `ok`
>   depends only on hard errors, so Submit stays live however loud the warning — including a
>   mistyped tank level giving a dose nine times the plant median. Rejected because turning an
>   advisory into a hard stop in a concrete room is how an operator learns to defeat the app, and
>   genuine operating exceptions have to stay possible. ⚠ The source comment that falsely claimed
>   flags were "acknowledged" was corrected 2026-09-02; **the comment was the defect, the behaviour
>   is intended.**
