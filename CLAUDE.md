...
- **⚠ Deploy gotcha reconfirmed:** the CivicScope webhook served a **stale serverless-function build** — new `api/*.js` code was in GitHub but the live function didn't update until a **`vercel deploy --prod --force`**. Going forward: after any change touching an `api/*.js` function, force-deploy or verify the live endpoint — don't trust the push alone. **UPDATE 2026-07-19 (worse + new remedy):** the webhook **skipped a push entirely** (commit `b315c2f` on GitHub, zero deployment created — smoke gate can't catch this, it tests the still-healthy old build), AND **CLI deploys from keith-1002 are BLOCKED by Vercel** (reason unknown; not spend — $9.69/$20). **Working fallback: Vercel REST API git-deploy** — `POST /v13/deployments?skipAutoDetectionConfirmation=1` body `{name:'civicscope', target:'production', gitSource:{type:'github', org:'anchoradvisorsnorth', repo:'civicscope', ref:'main'}}` (READY in ~90s). Verify api changes live via version markers: `api/football-pool.js` answers `GET ?ver=1` (bump its `VER` const on every edit) — after ANY deploy touching it, curl-check the live ver matches the repo.
