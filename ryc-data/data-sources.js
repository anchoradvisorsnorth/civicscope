...
// via <script src="/ryc-data/data-sources.js"> so there is ONE definition — edit here only.
...
function dataSourcesHTML() {
  return `<div class="ds-head"><div><h2>Data Sources &amp; Field Provenance</h2>`
    ...
    + `<div class="ds-note"><strong>⏱ STEP 0 — before reconciling ANYTHING, check the snapshot time.</strong> Dashboard/Command Foundation figures are a <strong>nightly snapshot (~5:00 AM ET)</strong> — anything posted in Foundation <em>after</em> that moment doesn't appear until the next morning's refresh. Comparing against live Foundation screens? First ask: <em>has accounting posted a batch since 5 AM?</em> <strong>Real example (Jul 13, 2026):</strong> a June month-end batch posted Sunday midday; four jobs then "mismatched" by exactly that batch (Shipshewana +$459,670.94 cost, Helix +$150,450.97 cost, Coldwater's $352,069.04 June pay app, LaCrosse's $14,000 → $11,568.21 invoice correction). Next morning's refresh tied to the penny with zero changes. Rule of thumb: <strong>a gap that equals exactly one batch or pay-app = timing, not error.</strong></div>`
    ...
    + `<tr><td><strong>Foundation</strong> (read-only ODBC)</td><td>nightly job → CRM database (dashboard) · <strong>live</strong> query (Foundation tool)</td><td>Dashboard = ~5:00am ET nightly · Query tool = real-time</td></tr>`
    ...
    + `<tr><td><strong>Contract Price</strong></td><td>Procore</td><td><strong>Procore Revised Contract Amount</strong> (<code>revised_contract_amount</code>) — flipped from Foundation per Tristan, Jul 2026 (approved COs hit Procore before Foundation accounting). Foundation <code>currentContract</code> (original + executed CO income) as fallback; ⚑ flags material divergence. <em>Raw Foundation <code>original_contract</code> will be lower.</em></td></tr>`
    ...
    + `<li><strong>Snapshot timing — check FIRST.</strong> Foundation figures are a nightly ~5:00am ET pull; batches posted during the day appear tomorrow. A gap of exactly one posting batch or pay-app = timing. See Step 0 above.</li>`
    + `<li><strong>Contract</strong> = Procore Revised Contract Amount (Foundation <code>currentContract</code> fallback); a raw Foundation <code>original_contract</code> is lower by the CO income.</li>`
    ...
    + `<h3>Reconciling a number back to Foundation</h3>`
    + `<div style="font-size:12px;margin:2px 0 6px"><strong>Protocol:</strong> ① note the snapshot time (Command: Data Trust view / Brief header; dashboard: header chips) → ② confirm no Foundation batches posted after it (or re-check tomorrow morning) → ③ match the definition from the table above (gross vs net, posted vs committed, Procore vs Foundation) → ④ only then treat a residual gap as a real exception and flag it.</div>`
    + `<table><thead><tr><th>Number</th><th>Where in Foundation</th></tr></thead><tbody>`
